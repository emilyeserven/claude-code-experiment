...
test.describe("Home page", () => {
  ...
  test("shows the timer at 00:00:00.000", async ({ page }) => {
    const display = page.getByTestId("timer-display");
    await expect(display).toHaveText("00:00:00.000");
  });

  test("has start, stop, and reset timer buttons", async ({ page }) => {
  ...
  test("start button begins the timer and stop button pauses it", async ({ page }) => {
    const display = page.getByTestId("timer-display");
    await expect(display).toHaveText("00:00:00.000");
    ...
    await expect(display).not.toHaveText("00:00:00.000", { timeout: 3000 });
  ...
  test("reset button returns the timer to zero", async ({ page }) => {
    const display = page.getByTestId("timer-display");
  ...
  test("start button is disabled while timer is running", async ({ page }) => {
    await page.getByTestId("timer-start-button").click();
  ...
  test("stop button is disabled while timer is not running", async ({ page }) => {
  ...
  test("shows empty entries message initially", async ({ page }) => {
    await expect(page.getByTestId("timer-entries-empty")).toBeVisible();
  });

  test("submitting an entry via the submit button adds a row to the table", async ({ page }) => {
    await page.getByTestId("timer-start-button").click();
  ...
  test("submitting an entry via Enter key adds a row to the table", async ({ page }) => {
    await page.getByTestId("timer-start-button").click();
  ...
  test("submitting empty input does not add a row", async ({ page }) => {
    await page.getByTestId("timer-submit-button").click();
    await expect(page.getByTestId("timer-entries-empty")).toBeVisible();
  });

  test("multiple entries appear in order", async ({ page }) => {
    await page.getByTestId("timer-start-button").click();
  ...
  test("entry timestamps are in HH:MM:SS.mmm format", async ({ page }) => {
    await page.getByTestId("timer-start-button").click();
