...
test.describe("About page", () => {
  ...
  test("displays the about heading", async ({
    page,
  }) => {
    await expect(page.getByText("Hello from About!")).toBeVisible();
  });

  test("shows a pending status while data is loading", async ({
    page,
  }) => {
  ...
  test("shows loaded status when the API responds successfully", async ({
    page,
  }) => {
  ...
  test("shows error status when the API request fails", async ({
    page,
  }) => {
    await page.route("**/api", (route) => {
      return route.fulfill({
    ...
    await expect(status).toContainText("Erroring", {
      timeout: 30_000,
    });
  });
});
